...
class CAModel(tf.keras.Model):
    ...
    def perceive(self, x, angle=0.0):
        ...
        dx = np.outer([1, 2, 1], [-1, 0, 1]) / 8.0  # dx: Sobel filter 'X' [[-1, -2, -1], [000], [1, 2, 1]]/8;
        dy = dx.T  # dx: Sobel filter 'X' [[1, 2, 1], [000], [-1, -2, -1]]/8;
        c, s = tf.cos(angle), tf.sin(angle)
        kernel = tf.stack([identify, c * dx - s * dy, s * dx + c * dy], -1)[:, :, None, :]  # todo: wtf ?!
